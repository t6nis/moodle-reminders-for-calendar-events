...
defined('REMINDERS_FIRST_CRON_CYCLE_CUTOFF_DAYS') or DEFINE('REMINDERS_FIRST_CRON_CYCLE_CUTOFF_DAYS', 2);

defined('REMINDERS_7DAYSBEFORE_INSECONDS') or DEFINE('REMINDERS_7DAYSBEFORE_INSECONDS', 7*24*3600);
defined('REMINDERS_3DAYSBEFORE_INSECONDS') or DEFINE('REMINDERS_3DAYSBEFORE_INSECONDS', 3*24*3600);
defined('REMINDERS_1DAYBEFORE_INSECONDS') or DEFINE('REMINDERS_1DAYBEFORE_INSECONDS', 24*3600);

defined('REMINDERS_SEND_ALL_EVENTS') or DEFINE('REMINDERS_SEND_ALL_EVENTS', 50);
defined('REMINDERS_SEND_ONLY_VISIBLE') or DEFINE('REMINDERS_SEND_ONLY_VISIBLE', 51);

defined('REMINDERS_ACTIVITY_BOTH') or DEFINE('REMINDERS_ACTIVITY_BOTH', 60);
defined('REMINDERS_ACTIVITY_ONLY_OPENINGS') or DEFINE('REMINDERS_ACTIVITY_ONLY_OPENINGS', 61);
defined('REMINDERS_ACTIVITY_ONLY_CLOSINGS') or DEFINE('REMINDERS_ACTIVITY_ONLY_CLOSINGS', 62);
...
class reminder_task extends \core\task\scheduled_task {
    ...
    /**
     * Taken from Moodle API, striped log event generation, we don't want that every send message event goes right into our log :-)
     *
     * Returns true if input string is empty/whitespaces only, otherwise false.
     *
     * @param type $str string
     ...
     * @return boolean true if string is empty or whitespace
     */
    protected function reminder_message_send($eventdata) {
        global $CFG, $DB;
     ...
     */
     public function fetch_module_instance($modulename, $instance, $courseid=0) {
        global $DB;
    ...
    public function execute() {
        ...
        foreach ($upcomingevents as $event) {
            ...
            try {
                ...
                switch ($event->eventtype) {
                    ...
                    case 'due':
                        ...
                        if (!$this->isemptyString($event->modulename)) {
                            $course = $DB->get_record('course', array('id' => $event->courseid));
                            $cm = get_coursemodule_from_instance($event->modulename, $event->instance, $event->courseid);

                            if (!empty($course) && !empty($cm)) {
                                $activityobj = $this->fetch_module_instance($event->modulename, $event->instance, $event->courseid);
                    ...
                    case 'group':
                        ...
                        if (!empty($group)) {
                            ...
                            //
                            if (!$this->isemptyString($event->modulename) && $event->courseid > 0) {
                                $activityobj = $this->fetch_module_instance($event->modulename, $event->instance, $event->courseid);
                                $reminder->set_activity($event->modulename, $activityobj);
                            }
                            $eventdata = $reminder->create_reminder_message_object($fromuser);
            ...
            foreach ($sendusers as $touser) {
                ...
                //mtrace("-----------------------------------");
                try {
                    $mailresult = $this->reminder_message_send($eventdata);
